  ...
  const fetchGeoLocations = useCallback(async () => {
    try {
      ...
      if (result.status === 200 && result.data) {
        ...
        console.error(
          `API Error on GET: Status ${result.status}, Data: ${JSON.stringify(result.data)}`
        );
      }
    } catch (err: any) {
  ...
  const updateGeoLocations = useCallback(async (newLocations: string[]) => {
    try {
      ...
      if (result.status === 200) {
        ...
        console.log(result.message || "Geo location configuration updated successfully");
      } else {
        console.error(
      ...
      console.error("Failed to update geo locations:", err.message || err);
    }
  }, []);

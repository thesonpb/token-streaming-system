...
export function usePolicyData() {
        ...
        try {
            ...
            if (data.status === 200 && data.policies) {
                const transformedPolicies =
                    data.policies.map(transformApiPolicy);
    ...
    const enablePolicy = useCallback(
            ...
            try {
                ...
                if (!response.ok) {
                    const errorData = await response.json().catch(() => ({
                        message:
                            "Failed to enable policy and parse error response",
                    }));
                    throw new Error(
                        errorData.message ||
        ...
        [
            setAllPolicies,
            setError /* fetchPolicies if used for error recovery */,
        ]
    );
        ...
        async (policyId: string) => {
            ...
            try {
                ...
                if (!response.ok) {
                    const errorData = await response.json().catch(() => ({
                        message:
                            "Failed to disable policy and parse error response",
                    }));
                    throw new Error(
                        errorData.message ||
                            `HTTP error! status: ${response.status}`
